...
export function abrirSpotlight() {
  ...
  const buscar = debounce((termo) => {
    ...
    let html = '';
    if (obras.length) { html += secao('Obras', '<i data-lucide="images"></i>') + obras.map(o => `<div class="spotlight-item" data-rota="catalogo" data-payload="${sanitizarHTML(o.id)}"><span class="si-icone" style="background-image:url('${sanitizarURL(o.imagem || '')}');background-size:cover;width:28px;height:28px;border-radius:4px;"></span><span>${sanitizarHTML(o.titulo)}</span><span class="si-info">${sanitizarHTML(o.tecnica || '')} · ${formatarMoeda(o.preco)}</span></div>`).join(''); }
    if (clientes.length) { html += secao('Clientes', '<i data-lucide="user"></i>') + clientes.map(c => `<div class="spotlight-item" data-rota="clientes"><span class="si-icone"><i data-lucide="user"></i></span><span>${sanitizarHTML(c.nome)}</span><span class="si-info">${sanitizarHTML(c.email || '')}</span></div>`).join(''); }
    if (vendas.length) { html += secao('Vendas', '<i data-lucide="dollar-sign"></i>') + vendas.map(v => `<div class="spotlight-item" data-rota="vendas"><span class="si-icone"><i data-lucide="dollar-sign"></i></span><span>Recibo ${sanitizarHTML(v.numeroRecibo || '')}</span><span class="si-info">${formatarMoeda(v.valorTotal || v.valor)}</span></div>`).join(''); }
    ...
    if (encomendas.length) { html += secao('Encomendas', '<i data-lucide="package"></i>') + encomendas.map(e => `<div class="spotlight-item" data-rota="encomendas"><span class="si-icone"><i data-lucide="package"></i></span><span>${sanitizarHTML(e.cliente || e.clienteNome || '')}</span><span class="si-info">${sanitizarHTML(e.descricao ? e.descricao.slice(0, 40) : '')}</span></div>`).join(''); }
