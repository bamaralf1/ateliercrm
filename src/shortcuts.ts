...
const ATALHOS_PADRAO = {
  ...
  'ctrl+s': { desc: 'Salvar dados', acao: () => { if (window.saveManager) window.saveManager.salvarAgora(); else { dataStore?.salvar(); mostrarToast('Dados salvos!', 'sucesso'); } activityLogger.registrar('atualizacao', 'Dados salvos', 'Salvamento manual via Ctrl+S', 'atualizacao'); } },
