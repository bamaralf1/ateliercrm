export class DashboardView extends BaseView {
  constructor(dataStore, router) {
    ...
    this.widgetsDisponiveis = [
      { id: 'producao', rotulo: 'Produção Mensal', icone: '<i data-lucide="chart-line"></i>', visivel: true },
      { id: 'tecnicas', rotulo: 'Técnicas', icone: '<i data-lucide="palette"></i>', visivel: true },
      { id: 'receita', rotulo: 'Receita', icone: '<i data-lucide="dollar-sign"></i>', visivel: true },
  ...
  render() {
    ...
    const kpis = [
      { rotulo: 'Total de Obras', valor: obras.length, tendencia: crescimentoMensal, icone: '<i data-lucide="images"></i>', cor: '#2563eb', sparkline: this.gerarSparkline(obras, 'criacao'), variacao: obras.length > 0 ? variacaoObras : null },
      { rotulo: 'Obras Vendidas', valor: vendidas.length, sub: `${obras.length > 0 ? ((vendidas.length / obras.length) * 100).toFixed(1) : 0}% do total`, icone: '<i data-lucide="check"></i>', cor: '#16a34a', sparkline: '' },
      { rotulo: 'Valor do Acervo', valor: formatarMoeda(valorAcervo), valorNum: valorAcervo, sub: `Ticket médio: ${formatarMoeda(ticketMedio)}`, icone: '<i data-lucide="dollar-sign"></i>', cor: '#d97706', sparkline: '' },
      { rotulo: 'Total Vendido', valor: formatarMoeda(valorVendido), valorNum: valorVendido, sub: `${receitaMes > 0 ? formatarMoeda(receitaMes) + ' este mês' : vendas.length + ' venda(s)'}`, icone: '<i data-lucide="bar-chart-3"></i>', cor: '#7c3aed', sparkline: this.gerarSparkline(vendas, 'receita'), variacao: variacaoReceita },
      { rotulo: 'Clientes', valor: clientes.length, sub: `${this.contarClientesAtivos(clientes)} ativos`, icone: '<i data-lucide="users" aria-hidden="true"></i>', cor: '#0891b2', sparkline: '' },
      { rotulo: 'Favoritas', valor: obrasFavoritas, sub: '<i data-lucide="star"></i> obras marcadas', icone: '<i data-lucide="star"></i>', cor: '#dc2626', sparkline: '' }
    ];
    ...
    return `
      <div class="view-cabecalho">
        ...
        <div class="dashboard-acoes">
          <button class="btn-gradient" id="btnDownloadDashboard" title="Baixar dashboard como imagem"><i data-lucide="camera"></i> Exportar</button>
           <button class="btn-secundario" id="btnConfigWidgets" title="Configurar widgets" aria-label="Configurar widgets">⚙️</button>
           <button class="btn-secundario" id="btnAtualizarDashboard" title="Atualizar dados" aria-label="Atualizar dados"><i data-lucide="refresh-cw"></i></button>
        </div>
      </div>
      ...
      <div class="widgets-toolbar">
        <span class="widgets-toolbar-dica"><i data-lucide="lightbulb"></i> Arraste os widgets para reordenar. Clique em ⚙️ para mostrar/ocultar.</span>
      </div>
      ...
      <div class="painel atalhos-rodape">
        ...
        <div class="atalhos-rapidos">
          ...
          <button class="btn-ghost" id="btnAtalhoClientes"><i data-lucide="user"></i> Gerenciar Clientes</button>
        </div>
      </div>
      ...
      <div class="previsao-container">
        ...
        <div class="previsao-barra-container">
          ...
          <div class="previsao-barra-item">
            ...
            <span class="previsao-numero-peq ${dados.tendencia > 0 ? 'positiva' : 'negativa'}">${formatarMoeda(Math.abs(dados.projecao6M))} ${dados.tendencia > 0 ? '<i data-lucide="chart-line"></i>' : '📉'}</span>
          </div>
        </div>
        <div class="previsao-detalhe">
          <span class="texto-ajuda">Baseado em regressão linear simples sobre os últimos meses</span>
          ${dados.tendencia > 0 ? `<span class="tag-status disponivel">Tendência positiva <i data-lucide="chart-line"></i></span>` : `<span class="tag-status vendida">Tendência negativa 📉</span>`}
        </div>
  ...
  renderWidgetNotificacoes() {
    ...
    materiais.forEach(m => {
      if (m.quantidade <= m.quantidadeMinima) {
        notificacoes.push({ tipo: 'estoque', gravidade: m.quantidade <= (m.quantidadeMinima || 0) / 2 ? 'alta' : 'media', icone: '<i data-lucide="triangle-alert"></i>', mensagem: `"${m.nome}" está com estoque crítico (${m.quantidade} ${m.unidade || 'un'})`, acao: 'Ir para Atelier', rota: 'atelier' });
      }
    });
    ...
    clientes.forEach(c => {
      if (c.ultimoContato || c.criadoEm) {
        ...
        if (dias > 60) {
          notificacoes.push({ tipo: 'cliente', gravidade: dias > 180 ? 'alta' : 'media', icone: '<i data-lucide="user"></i>', mensagem: `"${c.nome}" sem contato há ${dias} dias`, acao: 'Ver cliente', rota: 'clientes' });
        }
      }
    });
    ...
    eventos.forEach(e => {
      if (e.dataEvento) {
        ...
        if (dias > 0 && dias <= 60) {
          notificacoes.push({ tipo: 'evento', gravidade: dias <= 15 ? 'alta' : 'media', icone: '<i data-lucide="calendar"></i>', mensagem: `"${e.nome}" em ${dias} dias (${e.status})`, acao: 'Ver eventos', rota: 'exposicoes' });
        }
      }
    });
    ...
    obras.forEach(o => {
      if (o.historicoPrecos && o.historicoPrecos.length > 1) {
        ...
        if (ultimo.preco < penultimo.preco) {
          notificacoes.push({ tipo: 'preco', gravidade: 'media', icone: '<i data-lucide="tag"></i>', mensagem: `"${o.titulo}" teve redução de preço (${formatarMoeda(penultimo.preco)} → ${formatarMoeda(ultimo.preco)})`, acao: 'Ver obra', rota: 'catalogo' });
        }
      }
    });

    if (notificacoes.length === 0) {
      return '<div class="estado-vazio"><div class="icone-vazio"><i data-lucide="check"></i></div><p>Tudo em ordem! Nenhuma notificação pendente.</p></div>';
    }
      ...
      <div class="metas-container">
        <div class="meta-card">
          ...
          <div class="meta-footer">
            ...
            <span class="${pctMes >= 100 ? 'positiva' : ''}">${pctMes >= 100 ? '<i data-lucide="check"></i> Meta atingida!' : `Faltam ${formatarMoeda(metaMensal - faturamentoMes)}`}</span>
          </div>
          ${pctMes < 100 ? `<div class="meta-diaria"><i data-lucide="target"></i> Meta diária necessária: ${formatarMoeda(metaDiaria)}/dia (${diasRestantes} dias restantes)</div>` : ''}
        </div>
        <div class="meta-card">
          <div class="meta-header">
          ...
          <div class="meta-footer">
            ...
            <span class="${pctAno >= 100 ? 'positiva' : ''}">${pctAno >= 100 ? '<i data-lucide="check"></i> Parabéns!' : `Faltam ${formatarMoeda(metaAnual - faturamentoAno)}`}</span>
          </div>
        </div>
      </div>
    `;
  ...
  renderWidgetRecentes() {
    ...
    if (recentes.length === 0) {
      return '<div class="estado-vazio"><div class="icone-vazio"><i data-lucide="images"></i></div><p>Nenhuma obra cadastrada ainda.</p></div>';
    }
    return `
        ...
        ${recentes.map(o => {
          ...
          return `
            <li class="item-obra-recente">
              <div class="thumb-obra">${imgSrc ? `<img src="${srcMostra}"${attrIdb} alt="${o.titulo}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">` : '<i data-lucide="images"></i>'}</div>
  ...
  renderWidgetDica() {
    ...
    return `<div class="dica-card"><div class="dica-icone"><i data-lucide="lightbulb"></i></div><div class="dica-texto"><p>${dica}</p><span class="texto-ajuda">Dica do dia · Atualiza automaticamente</span></div></div>`;
  }
