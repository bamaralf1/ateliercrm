...
Only state can be modified.`);
  ...
  var DataStore = class {
    ...
    popularExemplos() {
      ...
      this.dados.materiais = [
        ...
        { id: "mat_6", nome: "Moldura Cl\xE1ssica 30\xD740", categoria: "molduras", subcategoria: "cl\xE1ssica", marca: "Molduraz", quantidade: 2, unidade: "un", quantidadeMinima: 4, local: "Dep\xF3sito", dataAquisicao: "2025-06-10", validade: "", precoUnitario: 65, foto: "", notas: '<i class="fas fa-exclamation-triangle"></i> ABAIXO DO M\xCDNIMO \u2014 repor urgente!' }
      ];
  ...
  var ActivityLogger = class {
    ...
    obterIcone(tipo) {
      const icones = { criacao: "\u2728", atualizacao: '<i class="fas fa-pen"></i>', exclusao: '<i class="fas fa-trash"></i>', venda: '<i class="fas fa-dollar-sign"></i>', favorita: '<i class="fas fa-star"></i>', export: '<i class="fas fa-file"></i>', import: "\u{1F4E5}", status: '<i class="fas fa-pencil-alt"></i>' };
  ...
  var Router = class {
    constructor(dataStore2) {
      ...
      this.rotas = {
        ...
        catalogo: { rotulo: "Cat\xE1logo", icone: ICONES.catalogo, render: () => catalogoView.render(), aposRender: () => catalogoView.aposRenderizar() },
        ...
        referencias: { rotulo: "Refer\xEAncias", icone: ICONES.referencias, render: () => referenciasView.render(), aposRender: () => referenciasView.aposRenderizar() },
        ...
        exposicoes: { rotulo: "Exposi\xE7\xF5es", icone: ICONES.exposicoes, render: () => exposicoesView.render(), aposRender: () => exposicoesView.aposRenderizar() },
        ...
        diario: { rotulo: "Di\xE1rio", icone: ICONES.diario, render: () => diarioView.render(), aposRender: () => diarioView.aposRenderizar() },
        ...
        configuracoes: { rotulo: "Configura\xE7\xF5es", icone: ICONES.configuracoes, render: () => configuracoesView.render(), aposRender: () => configuracoesView.aposRenderizar() },
  ...
  var DashboardView = class extends BaseView {
    constructor(dataStore2, router2) {
      ...
      this.widgetsDisponiveis = [
        { id: "producao", rotulo: "Produ\xE7\xE3o Mensal", icone: '<i class="fas fa-chart-line"></i>', visivel: true },
        { id: "tecnicas", rotulo: "T\xE9cnicas", icone: '<i class="fas fa-palette"></i>', visivel: true },
        ...
        { id: "notificacoes", rotulo: "Notifica\xE7\xF5es Inteligentes", icone: '<i class="fas fa-bell"></i>', visivel: true },
      ...
      <div class="previsao-container">
        ...
        <div class="previsao-barra-container">
          ...
          <div class="previsao-barra-item">
            ...
            <span class="previsao-numero-peq ${dados.tendencia > 0 ? "positiva" : "negativa"}">${formatarMoeda(Math.abs(dados.projecao6M))} ${dados.tendencia > 0 ? '<i class="fas fa-chart-line"></i>' : "\u{1F4C9}"}</span>
          </div>
        </div>
        <div class="previsao-detalhe">
          <span class="texto-ajuda">Baseado em regress\xE3o linear simples sobre os \xFAltimos meses</span>
          ${dados.tendencia > 0 ? `<span class="tag-status disponivel">Tend\xEAncia positiva <i class="fas fa-chart-line"></i></span>` : `<span class="tag-status vendida">Tend\xEAncia negativa \u{1F4C9}</span>`}
        </div>
    ...
    renderWidgetNotificacoes() {
      ...
      materiais.forEach((m) => {
        if (m.quantidade <= m.quantidadeMinima) {
          notificacoes.push({ tipo: "estoque", gravidade: m.quantidade <= (m.quantidadeMinima || 0) / 2 ? "alta" : "media", icone: '<i class="fas fa-exclamation-triangle"></i>', mensagem: `"${m.nome}" est\xE1 com estoque cr\xEDtico (${m.quantidade} ${m.unidade || "un"})`, acao: "Ir para Atelier", rota: "atelier" });
        }
      });
      clientes.forEach((c) => {
        if (c.ultimoContato || c.criadoEm) {
          ...
          if (dias > 60) {
            notificacoes.push({ tipo: "cliente", gravidade: dias > 180 ? "alta" : "media", icone: '<i class="fas fa-user"></i>', mensagem: `"${c.nome}" sem contato h\xE1 ${dias} dias`, acao: "Ver cliente", rota: "clientes" });
          }
        }
      });
      eventos.forEach((e) => {
      ...
      obras.forEach((o) => {
        if (o.historicoPrecos && o.historicoPrecos.length > 1) {
          ...
          if (ultimo.preco < penultimo.preco) {
            notificacoes.push({ tipo: "preco", gravidade: "media", icone: '<i class="fas fa-tag"></i>', mensagem: `"${o.titulo}" teve redu\xE7\xE3o de pre\xE7o (${formatarMoeda(penultimo.preco)} \u2192 ${formatarMoeda(ultimo.preco)})`, acao: "Ver obra", rota: "catalogo" });
          }
        }
      });
      if (notificacoes.length === 0) {
        return '<div class="estado-vazio"><div class="icone-vazio"><i class="fas fa-check"></i></div><p>Tudo em ordem! Nenhuma notifica\xE7\xE3o pendente.</p></div>';
      }
      return `
    ...
    renderWidgetMetas() {
      ...
      <div class="metas-container">
        <div class="meta-card">
          ...
          ${pctMes < 100 ? `<div class="meta-diaria"><i class="fas fa-bullseye"></i> Meta di\xE1ria necess\xE1ria: ${formatarMoeda(metaDiaria)}/dia (${diasRestantes} dias restantes)</div>` : ""}
        </div>
        <div class="meta-card">
          <div class="meta-header">
          ...
          <div class="meta-footer">
            ...
            <span class="${pctAno >= 100 ? "positiva" : ""}">${pctAno >= 100 ? '<i class="fas fa-check"></i> Parab\xE9ns!' : `Faltam ${formatarMoeda(metaAnual - faturamentoAno)}`}</span>
          </div>
        </div>
      </div>
    `;
    ...
    renderWidgetDica() {
      ...
      return `<div class="dica-card"><div class="dica-icone"><i class="fas fa-lightbulb"></i></div><div class="dica-texto"><p>${dica}</p><span class="texto-ajuda">Dica do dia \xB7 Atualiza automaticamente</span></div></div>`;
    }
    /* ---- Lifecycle ---- */
    aposRenderizar() {
      this.removerListeners();
  ...
  var CatalogoView = class extends BaseView {
    ...
    render() {
      ...
      <div class="view-cabecalho">
        ...
        <div class="catalogo-acoes">
          ...
          <button class="btn-secundario" id="btnImportacaoLote" title="Importar m\xFAltiplas obras"><i class="fas fa-camera"></i> Importar</button>
    ...
    renderBarraBulk() {
      ...
      <div class="bulk-actions-bar">
        ...
        <div class="bulk-buttons">
          ...
          <button class="btn-secundario" id="bulkExportarPDF"><i class="fas fa-file"></i> Cat\xE1logo PDF</button>
    ...
    renderFiltros(anos) {
      ...
      <div class="filtros-rapidos">
        ...
        <button class="chip-filtro ${this.filtroRapido === "recentes" ? "ativo" : ""}" data-filtro="recentes"><i class="fas fa-calendar-alt"></i> Este m\xEAs</button>
    ...
    abrirImportacaoLote() {
      abrirModal(`
      <h3><i class="fas fa-camera"></i> Importar M\xFAltiplas Obras</h3>
    ...
    `;
      ...
      <div class="modal-acoes">
        ...
        <button class="btn-primario" id="btnExportarComparacao"><i class="fas fa-file"></i> Exportar Compara\xE7\xE3o</button>
      </div>
    `);
  ...
  var ClientesView = class extends BaseView {
    ...
    render() {
      ...
      const conteudo = clientes.length ? this.modo === "lista" ? this.renderTabela(clientes) : this.renderCards(clientes) : `<div class="tabela-wrapper"><div class="estado-vazio"><div class="icone-vazio"><i class="fas fa-user"></i></div><p>Nenhum cliente encontrado.</p></div></div>`;
    ...
    `;
      ...
      <div class="view-cabecalho">
        ...
        <div class="barra-acoes-referencias">
          ...
          <button class="btn-primario" id="btnNovaReferencia"><i class="fas fa-plus"></i> Nova Refer\xEAncia</button>
        </div>
      </div>
  ...
  var GaleriaVirtualView = class {
    ...
    compartilhar() {
      ...
      const msg = `Ol\xE1! <i class="fas fa-palette"></i> Convido voc\xEA para um tour virtual pela minha galeria de obras:
${url}
...
Aprecie a exposi\xE7\xE3o!`;
  ...
  var PrecificadorView = class extends BaseView {
    ...
    render() {
      ...
      <div class="precificador" id="precificadorContainer">
        ...
        (i) => `<span class="estrela ${i <= this.calc.complexidade ? "preenchida" : ""}" data-val="${i}">\u2605</span>`
          ...
          <button class="btn-primario" id="btnSalvarPrecoCalc" style="margin-top:12px;width:100%;"><i class="fas fa-save"></i> Salvar pre\xE7o sugerido na obra</button>
        </div>
        ...
        ${proj.projecoes.map((p) => `
          <div class="projecao-card">
            ...
            <div class="proj-apreciacao ${p.aprecAnual > 0 ? "proj-positiva" : "proj-negativa"}">
              ${p.aprecAnual > 0 ? '<i class="fas fa-chart-line"></i>' : "\u{1F4C9}"} ${p.aprecAnual.toFixed(1)}% a.a.
            </div>
          </div>
        `).join("")}
      </div>
    ...
    renderHistoricoPrecos(obras) {
      ...
      return `
      <div class="card">
        <h3><i class="fas fa-clipboard"></i> Hist\xF3rico de Pre\xE7os</h3>
        <div style="margin-bottom:12px;">
    ...
    renderAnalise(obras, vendas) {
      ...
      <div class="analise-grid" style="margin-bottom:16px;">
        <div class="analise-card"><div class="analise-valor">${this.fmt(precoMedio)}</div><div class="analise-rotulo"><i class="fas fa-dollar-sign"></i> Pre\xE7o m\xE9dio</div></div>
        <div class="analise-card"><div class="analise-valor">${this.fmt(valorTotal)}</div><div class="analise-rotulo"><i class="fas fa-box"></i> Valor total do portf\xF3lio</div></div>
      ...
      <div class="mat-card">
        ...
        <div class="mat-acoes">
          ...
          <button data-acao="consumirMaterial" data-id="${m.id}">\u{1F4C9} Consumir</button>
           <button data-acao="excluirMaterial" data-id="${m.id}" style="color:#dc2626;" aria-label="Excluir material">\u{1F5D1}\uFE0F</button>
        </div>
      </div>
    `;
    }
      ...
      <table class="cons-table">
        ...
        ${rows.map((r) => `
          <tr>
            ...
            <td><button data-acao="excluirConsumo" data-id="${r.id}" style="font-size:0.7rem;padding:2px 6px;border:1px solid var(--border);background:var(--bg);color:#dc2626;cursor:pointer;" aria-label="Excluir consumo">\u{1F5D1}\uFE0F</button></td>
          </tr>
        `).join("")}
      </table>`}
    `;
    ...
    renderItemCompra(m, comprado) {
      ...
      <li class="${comprado ? "comprado" : ""}">
        ...
        <div class="lc-acoes">
          ...
          <button data-acao="removerLista" data-id="${m.id}" style="color:#dc2626;" aria-label="Remover da lista">\u{1F5D1}\uFE0F</button>
        </div>
      </li>
    `;
    }
        ...
        return `
            <div class="forn-card">
              ...
              <div class="forn-acoes">
                ...
                <button data-acao="excluirFornecedor" data-id="${f.id}" style="color:#dc2626;" aria-label="Excluir fornecedor">\u{1F5D1}\uFE0F</button>
              </div>
            </div>
          `;
      }).join("")}
...
${this.catLabels[d.categoria] || d.categoria || ""}${d.instituicao ? "\n" + d.instituicao : ""}${d.nivelRelacionamento ? "\nRelacionamento: " + "\u2605".repeat(d.nivelRelacionamento) : ""}`);
  ...
  var DiarioView = class extends BaseView {
    ...
    render() {
      ...
      const tabLabels = { entradas: '<i class="fas fa-clipboard"></i> Entradas', cronograma: '<i class="fas fa-calendar-alt"></i> Cronograma', processo: '<i class="fas fa-pencil-alt"></i> Processo', estatisticas: '<i class="fas fa-chart-bar"></i> Estat\xEDsticas', inspiracao: '<i class="fas fa-lightbulb"></i> Inspira\xE7\xE3o' };
      const content = {
      ...
      <div class="diario-header">
        <div>
          <h2><i class="fas fa-clipboard"></i> Di\xE1rio Criativo</h2>
    ...
    renderEntradas() {
      ...
      ${entradas.length === 0 ? '<div class="diario-card" style="text-align:center;padding:30px;color:var(--text-muted);"><p style="font-size:1.2rem;margin-bottom:6px;"><i class="fas fa-pencil-alt"></i></p><p>Nenhuma entrada no di\xE1rio ainda.<br>Clique em "Nova Entrada" para come\xE7ar seu registro criativo.</p></div>' : ""}
    ...
    renderEstatisticas() {
      ...
      <div class="stats-grid">
        ...
        <div class="stats-card">
          <h4><i class="fas fa-calendar-alt"></i> M\xE9dia Di\xE1ria</h4>
        ...
        <div class="stats-card">
          <h4><i class="fas fa-pencil-alt"></i> M\xE9dia p/ Obra</h4>
        ...
        ${tecArray.length > 0 ? `
        <div class="stats-card" style="grid-column:1/-1;">
          <h4><i class="fas fa-pencil-alt"></i> Produtividade por T\xE9cnica</h4>
        ...
        <div class="stats-card">
          <h4><i class="fas fa-palette"></i> Por T\xE9cnica \u2014 Dias M\xE9dios</h4>
      ...
      <div style="margin-top:14px;font-size:0.8rem;color:var(--text-muted);">
        <i class="fas fa-lightbulb"></i> Registre entradas di\xE1rias com humor e horas para estat\xEDsticas mais precisas.
      </div>
    `;
    }
    // ======================= 5. INSPIRAÇÃO DO DIA =======================
    renderInspiracao() {
      ...
      <div class="inspiracao-card">
        ...
        <div class="ic-prompt"><i class="fas fa-lightbulb"></i> Prompt criativo de hoje: <strong>${prompt2}</strong></div>
        ${desafio ? `<div class="ic-desafio"><i class="fas fa-bullseye"></i> Desafio da semana: ${desafio}</div>` : ""}
      </div>
      <div style="margin-top:16px;">
        <button class="btn-primario" id="btnNovaCitacao" style="font-size:0.8rem;padding:6px 14px;"><i class="fas fa-plus"></i> Nova cita\xE7\xE3o</button>
      ...
      <div style="margin-top:24px;">
        <h4 style="font-size:0.9rem;margin-bottom:8px;"><i class="fas fa-clipboard"></i> Todas as cita\xE7\xE3o</h4>
    ...
    abrirFormEntrada(id = null, dataPrefill = null) {
      ...
      const obras = this.obras;
      const obraOpts = obras.map((o) => `<option value="${o.id}"><i class="fas fa-images"></i> ${o.titulo || "Sem t\xEDtulo"}</option>`).join("");
    ...
    exportarProcessoPDF() {
      ...
      etapas.forEach((et, i) => {
        ...
        doc.setFontSize(8);
        if (et.data) {
          doc.text(`<i class="fas fa-calendar-alt"></i> ${new Date(et.data).toLocaleDateString("pt-BR")}`, margem, y);
          y += 4;
        }
        if (et.descricao) {
        ...
        if (et.notasTecnicas) {
          ...
          doc.text(`<i class="fas fa-pencil-alt"></i> T\xE9cnica: ${et.notasTecnicas}`, margem + 2, y);
          y += 5;
        }
      ...
      if (obra.serie) {
        doc.text(`<i class="fas fa-folder"></i> S\xE9rie: ${obra.serie}`, margem, y);
        y += 4;
      }
      if (obra.descricao) {
  ...
  var PortalView = class extends BaseView {
    ...
    renderEncomendaCard(enc) {
      const statusMap = {
        ...
        "aprovacao": { rotulo: "Aguardando Aprova\xE7\xE3o", cor: "#8b5cf6", icone: '<i class="fas fa-check"></i>' },
...
Ou hospede no Vercel arrastando o arquivo para vercel.com/new`;
  ...
  var ExportImportView = class extends BaseView {
    ...
    renderExportar() {
      ...
      const cards = colecoes.map((c) => {
        const rotulos = { obras: '<i class="fas fa-images"></i> Obras', clientes: '<i class="fas fa-user"></i> Clientes', vendas: '<i class="fas fa-dollar-sign"></i> Vendas', encomendas: '<i class="fas fa-box"></i> Encomendas', contatosProfissionais: "\u{1F91D} Contatos", interacoes: "\u{1F4AC} Intera\xE7\xF5es", eventos: "\u{1F3AA} Eventos", financas: '<i class="fas fa-chart-line"></i> Finan\xE7as' };
        return `
    ...
    renderPreview() {
      if (!this.previewData || !this.previewData.valido) {
        return `<div class="ei-preview-box ei-preview-erro"><span><i class="fas fa-times"></i></span> Arquivo inv\xE1lido: ${this.previewData?.erro || "formato n\xE3o reconhecido"}</div>`;
      }
      const linhas = this.previewData.colecoes.map(
        (c) => `<tr><td>${c.nome}</td><td>${c.quantidade}</td><td>${c.quantidade > 0 ? '<i class="fas fa-plus-circle"></i> Novos dados' : "\u2014"}</td></tr>`
      ).join("");
      ...
      <div class="ei-preview-box">
        ...
        <div class="ei-import-acoes">
          <button class="btn-primario" id="eiConfirmarImport"><i class="fas fa-check"></i> Confirmar Importa\xE7\xE3o</button>
    ...
    configurarAcoesImport(jsonTexto) {
      document.getElementById("eiConfirmarImport")?.addEventListener("click", () => {
        ...
        if (resultado.sucesso) {
          ...
          mostrarToast(`<i class="fas fa-times"></i> Erro na importa\xE7\xE3o: ${resultado.erro}`, "erro");
        }
      });
    ...
    actions: {
      ...
      _persistir() {
        try {
          ...
          try {
            if (window.dataStore) {
              window.dataStore.dados.obras = this.items;
            }
          } catch (e) {
    ...
    actions: {
      ...
      _persistir() {
        try {
          ...
          try {
            if (window.dataStore) {
              window.dataStore.dados.clientes = this.items;
            }
          } catch (e) {
    ...
    actions: {
      ...
      _persistir() {
        try {
          ...
          try {
            if (window.dataStore) {
              window.dataStore.dados.vendas = this.items;
            }
          } catch (e) {
    ...
    actions: {
      ...
      salvar() {
        try {
          ...
          try {
            if (window.dataStore) {
              window.dataStore.dados.config = this.$state;
            }
          } catch (e) {
  ...
  `).join("") : `<div class="estado-vazio"><div class="icone-vazio"><i class="fas fa-images"></i></div><p>Nenhuma obra cadastrada ainda. Clique em "Nova Obra" para come\xE7ar.</p></div>`;
    return `
    <div class="view-cabecalho">
      <div>
        <h2>Dashboard</h2>
    ...
    <div class="grid-painel">
      <div class="painel"><h3><i class="fas fa-chart-bar"></i> Produtividade Mensal</h3><div class="grafico-container">${graficoSvg}</div><div class="grafico-legenda"><span class="leg-item"><i class="fas fa-chart-bar"></i> Obras criadas por m\xEAs</span></div></div>
      <div class="painel"><h3><i class="fas fa-palette"></i> T\xE9cnicas Mais Usadas</h3><div class="tecnicas-container">${tecnicaMaisComum.length > 0 ? tecnicaMaisComum.map((t, i) => `<div class="barra-tecnica"><div class="tecnica-nome">${capitalizarTexto(t.tecnica)}</div><div class="tecnica-barra-wrapper"><div class="tecnica-barra" style="width: ${t.porcentagem}%"></div></div><div class="tecnica-valor">${t.quantidade} (${t.porcentagem.toFixed(0)}%)</div></div>`).join("") : '<div class="estado-vazio"><p>Sem dados suficientes</p></div>'}</div></div>
    </div>
    <div class="grid-painel">
    ...
    <div class="painel"><h3>\u26A1 Atalhos r\xE1pidos</h3><div class="atalhos-rapidos"><button class="btn-primario" id="btnAtalhoNovaObra">\u271A Nova Obra</button><button class="btn-secundario" id="btnAtalhoVenda">\u271A Nova Venda</button><button class="btn-secundario" id="btnAtalhoRecibo">\u{1F9FE} Gerar Recibo</button><button class="btn-secundario" id="btnAtalhoClientes"><i class="fas fa-user"></i> Gerenciar Clientes</button></div></div>
  `;
  }
...
<\/script>
  ...
  function abrirSpotlight() {
    ...
    const buscar = debounce2((termo) => {
      ...
      if (clientes.length) {
        html += secao("Clientes", '<i class="fas fa-user"></i>') + clientes.map((c) => `<div class="spotlight-item" data-rota="clientes"><span class="si-icone"><i class="fas fa-user"></i></span><span>${c.nome}</span><span class="si-info">${c.email || ""}</span></div>`).join("");
      }
      if (vendas.length) {
        html += secao("Vendas", '<i class="fas fa-dollar-sign"></i>') + vendas.map((v) => `<div class="spotlight-item" data-rota="vendas"><span class="si-icone"><i class="fas fa-dollar-sign"></i></span><span>Recibo ${v.numeroRecibo || ""}</span><span class="si-info">${formatarMoeda(v.valorTotal || v.valor)}</span></div>`).join("");
      }
      if (contatos.length) {
      ...
      if (encomendas.length) {
        html += secao("Encomendas", '<i class="fas fa-box"></i>') + encomendas.map((e) => `<div class="spotlight-item" data-rota="encomendas"><span class="si-icone"><i class="fas fa-box"></i></span><span>${e.cliente}</span><span class="si-info">${e.descricao ? e.descricao.slice(0, 40) : ""}</span></div>`).join("");
      }
      if (eventos.length) {
  ...
  var tourPassos = [
    ...
    { alvo: '[data-rota="diario"]', titulo: '<i class="fas fa-book-open"></i> Di\xE1rio Criativo', desc: "Registre seu processo di\xE1rio.", pos: "right" },
  ...
  (function() {
    ...
    function renderizarNotif() {
      ...
      if (atividades.length === 0) {
        lista.innerHTML = '<div class="notif-vazio"><i class="fas fa-bell"></i> Nenhuma notifica\xE7\xE3o ainda.</div>';
        return;
      }
  ...
  (function() {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = '<div class="gdo-content"><div class="gdo-icon"><i class="fas fa-camera"></i></div><div class="gdo-text">Solte para adicionar imagens</div><div class="gdo-hint">JPG \xB7 PNG \u2014 M\xFAltiplos arquivos</div></div>';
      document.body.appendChild(overlay);
    }
    let dropTimer = 0;
    ...
    document.addEventListener("drop", (e) => {
      ...
      if (imagens.length === 0) {
        mostrarToast('<i class="fas fa-exclamation-triangle"></i> Apenas imagens (JPG/PNG) s\xE3o suportadas.', "erro");
        return;
      }
      if (imagens.length === 1) {
        router?.navegar("catalogo");
